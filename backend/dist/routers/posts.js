...
export const postsRouter = router({
    ...
    create: procedure
        ...
        if (input.categoryIds.length) {
            await ctx.db.insert(postCategories).values(input.categoryIds.map((categoryId) => ({
                postId: id,
                categoryId
            })));
        }
    ...
    update: procedure
        ...
        if (rest.content !== undefined)
            values.content = rest.content;
        if (rest.excerpt !== undefined)
            values.excerpt = rest.excerpt;
        if (rest.authorId !== undefined)
            values.authorId = rest.authorId;
        if (rest.status !== undefined)
            values.status = rest.status;
        if (rest.scheduledFor !== undefined)
            values.scheduledFor = rest.scheduledFor;
